...
# What time did the top 100 Youtuber videos upload?
...
The different times of day used is presented in military time or the 24-hour clock and it is defined below: 
...
```{r top-100-time-of-day graph, echo = FALSE}
Time_of_day_n %>%
...
# Clickbait - Descending order of views
>>>>>>> 272293b9c6e2233588025535e64cf3ccded2d735
...
# Clickbait - Descending order of dislikes

```{r question-exclamation, echo=FALSE}
USvideos %>% 
  filter_all(any_vars(str_detect(str_to_lower(.), "\\?|\\!"))) %>%
  select(title, dislikes, views, likes, comment_count) %>%
  arrange(desc(dislikes))
```

- By comparison between the descending order of views and dislikes of "Clickbait", it is clear that the first one "P!nk - Beautiful Trauma (Official Video)" with most views also gets the most dislikes.
---
