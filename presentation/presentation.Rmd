---
title: "What is the most popular video on Youtube?."
subtitle: "Infinite possibilites"
...
sapply(str_extract_all(USvideos, "\\b[A-Z]+\\b"), paste, collapse= ' ')
```

```{r plot-iris, echo = FALSE}
ggplot(data = USvideos, mapping = aes(x = views, y = likes, color = comment_count)) +
  geom_point() + 
  theme_minimal()
```

```{r new dates, echo = FALSE}
USvideos$diff_in_days = as.numeric(difftime(USvideos$publish_time, USvideos$trending_date, units = "days")) 


```

```{r difference in days, echo = FALSE}
