...
- Broom, Simplifies our data and makes it easier to report
...
## What is the most frequent timing of a day for top-100-popular videos to be published?
...
```{r top-100-time-of-day, echo = FALSE}
USvideos %>%
  arrange(desc(views)) %>%
  slice(1:100) %>%
  count(Time_of_day) %>%
...
USvideos %>% 
  filter(title == "FIRST TIME IM DOING THIS! TALKS WITH LIZA." | title == "FLIPPING OVER SUPERCAR! *GONE VERY WRONG*" | title == "TRANSFORMING SUMMER MCKEEN INTO ME!!" | title == "WE WANT TO TALK ABOUT OUR MARRIAGE" | title == "TOTAL FAIL! NATASHA DENONA HOLIDAY WTF" | title == "FACE SURGERY WITH MAKEUP" | title == "NEWBORN PETS AND A NEW APARTMENT" | title == "SHOPPING FOR NEW FISH!!!" | title == "I TOOK THE $3,000,000 LAMBO TO CARMAX! They offered me......" | title == "ICE PRIMER? DOES IT WORK?!") %>%
...
```{r popular-categories, echo=FALSE, warning=FALSE}
USvideos %>%
...
$$Y_i = 4355 + \frac{3}{100}(7828792160) $$
- If and when the whole human population were to watch one video how many likes would it recieve?
